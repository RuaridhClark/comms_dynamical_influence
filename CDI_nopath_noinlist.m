function [C,x,S_vals] = CDI_nopath_noinlist(Incl,V,D)
...
%     [G,Nra,M] = graph_setup(M,Type);
    ...
    [S,x,I] = eig_embedding(Incl,V,D);

    [leaders,sdot_g,Ssave] = find_leaders(S,x,Nra,I); 
...
function [S,x,I] = eig_embedding(Incl,V,D)
%     [V,D]=eig(M');                  % calculates the left eigenvectors

    V = V(:, Incl); % Select eigenvectors from num_exclude+1 to the end
    D = D(:, Incl); % Select eigenvectors from num_exclude+1 to the end
...
function [leaders,sdot_g,Ssave] = find_leaders(S,x,Nra,I)
    % I - sorted indices by S magnitude

    range = 410;
    
    [~, indices] = maxk(S, range);   % only take the top 100 nodes in terms of dist to origin
...
%             Vt = x(:,in_list{cn});  % position vector for all incoming connections
        Vt = x(:,indices(indices~=indices(i)));  % position vector for all incoming connections
